...
// --- FMA Deployment (requester pod) ---

// FMADeploymentOption is a functional option for configuring the FMA requester Deployment.
type FMADeploymentOption func(*appsv1.Deployment)

// WithFMANodeSelector sets a nodeSelector on the requester pod template.
func WithFMANodeSelector(nodeSelector map[string]string) FMADeploymentOption {
...
func WithFMAImagePullPolicy(policy corev1.PullPolicy) FMADeploymentOption {
	return func(d *appsv1.Deployment) {
...
func buildFMADeployment(namespace, name, iscName, requesterImage string, replicas int32, opts ...FMADeploymentOption) *appsv1.Deployment {
...
// CreateFMADeployment creates the FMA requester Deployment.
func CreateFMADeployment(ctx context.Context, k8sClient kubernetes.Interface, namespace, name, iscName, requesterImage string, replicas int32, opts ...FMADeploymentOption) error {
	d := buildFMADeployment(namespace, name, iscName, requesterImage, replicas, opts...)
	_, err := k8sClient.AppsV1().Deployments(namespace).Create(ctx, d, metav1.CreateOptions{})
	return err
}

// DeleteFMADeployment deletes the FMA requester Deployment. Idempotent.
func DeleteFMADeployment(ctx context.Context, k8sClient kubernetes.Interface, namespace, name string) error {
	err := k8sClient.AppsV1().Deployments(namespace).Delete(ctx, name, metav1.DeleteOptions{})
	if err != nil && !errors.IsNotFound(err) {
		return fmt.Errorf("delete FMA Deployment %s: %w", name, err)
	}
	return nil
}

// ScaleFMADeployment patches the replica count of the FMA requester Deployment.
func ScaleFMADeployment(ctx context.Context, k8sClient kubernetes.Interface, namespace, name string, replicas int32) error {
	scale, err := k8sClient.AppsV1().Deployments(namespace).GetScale(ctx, name, metav1.GetOptions{})
	if err != nil {
		return fmt.Errorf("get scale for Deployment %s: %w", name, err)
	}
	scale.Spec.Replicas = replicas
	_, err = k8sClient.AppsV1().Deployments(namespace).UpdateScale(ctx, name, scale, metav1.UpdateOptions{})
	if err != nil {
...
// WaitForFMARequesterReady waits until the Deployment has the desired number of ready replicas.
func WaitForFMARequesterReady(ctx context.Context, k8sClient kubernetes.Interface, namespace, deployName string, replicas int32, timeout time.Duration) error {
	deadline := time.Now().Add(timeout)
	for time.Now().Before(deadline) {
		d, err := k8sClient.AppsV1().Deployments(namespace).Get(ctx, deployName, metav1.GetOptions{})
		if err != nil {
	...
	return fmt.Errorf("timed out waiting for Deployment %s to have %d ready replicas", deployName, replicas)
}

// WaitForFMAScaleDown waits until the Deployment has 0 replicas.
func WaitForFMAScaleDown(ctx context.Context, k8sClient kubernetes.Interface, namespace, deployName string, timeout time.Duration) error {
	deadline := time.Now().Add(timeout)
	for time.Now().Before(deadline) {
		d, err := k8sClient.AppsV1().Deployments(namespace).Get(ctx, deployName, metav1.GetOptions{})
		if err != nil {
	...
	return fmt.Errorf("timed out waiting for Deployment %s to scale down to 0", deployName)
}
